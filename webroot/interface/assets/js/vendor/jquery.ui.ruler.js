(function ($) {
  $.widget("ef.ruler", {
    ...
    _create: function () {
      ...
      /* Top ruler */
      var $topRuler = $(document.createElement("div"))
        .addClass("ruler")
        .addClass("top");
      ...
      /* Left ruler */
      var $leftRuler = $(document.createElement("div"))
        .addClass("ruler")
        .addClass("left");
      $leftRuler.appendTo($container);

      var $leftArrow = $(document.createElement("div")).addClass(
        leftarrowClass
      );
    ...
    _fixRulerSize: function () {
      ...
      this._$leftRuler.height(
        hContainer < hStage ? hStage : hContainer - this._scrollBarWidth
      );
    ...
    _updateRulerTicks: function (reset) {
      ...
      while (this._lastTopRulerPos < topRulerWidth) {
        if (
          this.options.tickMajor > 0 &&
          unitPos % this.options.tickMajor === 0
        ) {
          ...
          if (this.options.showLabel === true) $tick.text(unitPos);

          $tick.appendTo(this._$topRuler);
        } else if (
          this.options.tickMinor > 0 &&
          unitPos % this.options.tickMinor === 0
        ) {
          $tick = $(document.createElement("div"))
            .addClass("tick")
            .addClass("minor")
          ...
          unitPos % this.options.tickMicro === 0
        ) {
          $tick = $(document.createElement("div"))
            .addClass("tick")
            .addClass("micro")
    ...
    _fixArrowsPosition: function (mouseX, mouseY) {
      var arrowX =
        ...
        this._$corner.width() -
        Math.round(this._$topArrow.outerWidth() / 2) +
        parseInt($(".ef-ruler").parents().eq(1).css("padding-left"), 10);
      var arrowY =
        mouseY -
        this.element.offset().top -
        this._$corner.height() -
        Math.round(this._$leftArrow.outerHeight() / 2) +
        parseInt($(".ef-ruler").parents().eq(1).css("padding-top"), 10);

      this._$topArrow.css(
        "left",
        arrowX + this._scrollLeft + $(window).scrollLeft()
      );
      this._$leftArrow.css(
        "top",
        arrowY + this._scrollTop + $(window).scrollTop()
      );
    },

 ...
 * @var Subscription[]|CollectionInterface $allSubscriptions
 * @var Subscription[]|CollectionInterface $myAllowedSubscriptions
 * @var Subscription[]|CollectionInterface $myActiveSubscriptions
 */
...
$this->set('headerIcon', __("person-fill-gear"));
        ...
        <div class="card-body">
            <div class="users form content">
                <fieldset>
                    ...
                    <?php
                    if ($myAllowedSubscriptions->count() > 0) {
                        ?>
                        <h3 class="mt-5">Subscription Settings</h3>
                        <?php
                        $myActiveSubscriptionMap = [];
                        foreach ($myActiveSubscriptions as $myActiveSubscription) {
                            $myActiveSubscriptionMap[] = $myActiveSubscription->id;
                        }
                        ?>
                        <div class="row mt-0">
                            <div class="col-12 mt-0">
                                ...
                                foreach ($myAllowedSubscriptions as $subscription) {
                                    $textDisplay = "$subscription->name:  $subscription->description";
                                    $opts = [
                                        'class' => 'form-check-input mb-4',
                                        'label' => ['class' => 'form-check-label mb-1', 'text' => $textDisplay, 'style' => 'width: 100%',],
                                        'data-type' => 'boolean',
                                        'type' => 'checkbox',
                                        'checked' => in_array($subscription->id, $myActiveSubscriptionMap),
                                    ];
                                    $fieldId = "subscriptions.{$subscription->id}";
                                    echo $this->Form->control($fieldId, $opts);
                                }
                        ...
                        $fieldId = "subscriptions";
                        $opts = [
                            'value' => '',
                            'data-type' => 'boolean',
                        ];
                        echo $this->Form->hidden($fieldId, $opts);
                    }
                    ?>

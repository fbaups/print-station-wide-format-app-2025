...
$rows = [];
$counter = 0;
$impersonateUrl = ['controller' => 'Users', 'action' => 'impersonate'];
$allowImpersonation = $this->AuthUser->hasAccess($impersonateUrl);
?>
<?php foreach ($users as $user): ?>
    ...
    $impersonateLinkText = '';
    if ($impersonateUrl) {
        $impersonateLinkText = " | " . $this->Form->postLink(__('Impersonate'), ['controller' => 'Users', 'action' => 'impersonate', $user->id], ['confirm' => __('Are you sure you want to impersonate {0} {1}?', $user->first_name, $user->last_name)]);
    }
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('User Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New User'), ['action' => 'add'],
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-sm dataset" style="width:100%">
                        ...
                        <tr class="filter-headers">
                            ...
                            <th data-db-type="<?= $typeMap['user_statuses_id'] ?>"><?= Inflector::humanize('user_statuses_id') ?></th>

 ...
 * @var AppView $this
 * @var array $typeMap
 * @var FooTag[]|CollectionInterface $fooTags
...
$this->set('headerTitle', __('Foo Tags'));
...
$counter = 0;
?>
<?php foreach ($fooTags as $fooTag): ?>
    <?php $rows[$counter][] = $this->Number->format($fooTag->id) ?>
    <?php $rows[$counter][] = h($fooTag->name) ?>
    <?php
    $appendName = ($fooTag->name) ? ": $fooTag->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$fooTag->id}{$appendName}",
        'data-record-id' => $fooTag->id,
    ];
    $rows[$counter][] =
        $this->Html->link(__('Preview'), "#", $previewLinkOptions) . " | " .
        $this->Html->link(__('View'), ['action' => 'view', $fooTag->id]) . " | " .
        $this->Html->link(__('Edit'), ['action' => 'edit', $fooTag->id]) . " | " .
        $this->Form->postLink(__('Delete'), ['action' => 'delete', $fooTag->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fooTag->id)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Foo Tag Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Foo Tag'), ['action' => 'add'],

...
$this->set('headerTitle', __('Hot Folder Tokens'));
...
<?php foreach ($seeds as $seed): ?>
    ...
    $rows[$counter][] =
        $this->Form->postLink(__('Delete'), ['action' => 'token-delete', $seed->token], ['confirm' => __('Are you sure you want to delete Token {0}?', $seed->token)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card mb-5">
        ...
        <div class="card-body">
            <div class="hot-folder-tokes index content">
                <p>
                    <?php
                    $fullSubmitUrl = \Cake\Routing\Router::fullBaseUrl() . "/hot-folders/submit/hot_folder_name"
                    ?>
                    Web submission into a Hot Folder requires a Bearer Token to be present in the request header or
                    query-string.
                    <br/>
                ...
                <p>
                    Tokens should only be given to trusted entities as they allow files to be uploaded
                    into <?= APP_NAME ?>.
                </p>
            </div>
        </div>
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Token Listing') ?>
            <?= $this->Form->postLink('<i class="fas fa-plus"></i>' . __('&nbsp;New Token'), ['action' => 'token-add'],
                ['class' => 'btn btn-secondary btn-sm float-end', 'escape' => false]); ?>
            <?= $this->Html->link(__('Go To Hot Folders'), ['action' => 'index'],

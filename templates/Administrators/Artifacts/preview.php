...
<div class="record-preview">
    <dl class="row mb-0">
        ...
        if (is_file($recordData->sample_unc_preview)) {
            ...
            $mediaTypeText = 'Preview';
            if (in_array($recordData->mime_type, $pdfMimeTypes)) {
                $mediaTypeText = 'PDF Preview';
            } elseif (in_array($recordData->mime_type, $imageMimeTypes)) {
                $mediaTypeText = 'Image Preview';
            } elseif (in_array($recordData->mime_type, $videoMimeTypes)) {
                $mediaTypeText = 'Video Preview';
            }
            ?>
            <dt class="col-sm-3 text-truncate"><?= $mediaTypeText ?></dt>
            <dd class="col-sm-9">
            ...
            if (in_array($recordData->mime_type, $allowed)) {
                $recordData->createSampleSizesErrand();
                ?>
                <dt class="col-sm-3 text-truncate">Preview</dt>
                <dd class="col-sm-9">
                    A preview image is currently being generated, please try again is a few seconds.
                </dd>
                <?php
            }
        }

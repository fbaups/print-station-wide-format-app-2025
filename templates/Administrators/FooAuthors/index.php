 ...
 * @var AppView $this
 * @var array $typeMap
 * @var FooAuthor[]|CollectionInterface $fooAuthors
...
$this->set('headerTitle', __('Foo Authors'));
...
$counter = 0;
?>
<?php foreach ($fooAuthors as $fooAuthor): ?>
    <?php $rows[$counter][] = $this->Number->format($fooAuthor->id) ?>
    <?php $rows[$counter][] = h($fooAuthor->name) ?>
    <?php $rows[$counter][] = $this->Text->boolToWord($fooAuthor->is_active) ?>
    <?php
    $appendName = ($fooAuthor->name) ? ": $fooAuthor->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$fooAuthor->id}{$appendName}",
        'data-record-id' => $fooAuthor->id,
    ];
    $rows[$counter][] =
        $this->Html->link(__('Preview'), "#", $previewLinkOptions) . " | " .
        $this->Html->link(__('View'), ['action' => 'view', $fooAuthor->id]) . " | " .
        $this->Html->link(__('Edit'), ['action' => 'edit', $fooAuthor->id]) . " | " .
        $this->Form->postLink(__('Delete'), ['action' => 'delete', $fooAuthor->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fooAuthor->id)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Foo Author Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Foo Author'), ['action' => 'add'],

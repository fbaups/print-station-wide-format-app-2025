 ...
 * @var AppView $this
 * @var array $typeMap
 * @var FooRating[]|CollectionInterface $fooRatings
...
$this->set('headerTitle', __('Foo Ratings'));
...
$counter = 0;
?>
<?php foreach ($fooRatings as $fooRating): ?>
    <?php $rows[$counter][] = $this->Number->format($fooRating->id) ?>
    <?php $rows[$counter][] = $fooRating->has('foo_recipe') ? $this->Html->link($fooRating->foo_recipe->name, ['controller' => 'FooRecipes', 'action' => 'view', $fooRating->foo_recipe->id]) : '' ?>
    <?php $rows[$counter][] = $fooRating->score === null ? '' : $this->Number->format($fooRating->score) ?>
    <?php
    $appendName = ($fooRating->name) ? ": $fooRating->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$fooRating->id}{$appendName}",
        'data-record-id' => $fooRating->id,
    ];
    $rows[$counter][] =
        $this->Html->link(__('Preview'), "#", $previewLinkOptions) . " | " .
        $this->Html->link(__('View'), ['action' => 'view', $fooRating->id]) . " | " .
        $this->Html->link(__('Edit'), ['action' => 'edit', $fooRating->id]) . " | " .
        $this->Form->postLink(__('Delete'), ['action' => 'delete', $fooRating->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fooRating->id)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Foo Rating Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Foo Rating'), ['action' => 'add'],
                ['class' => 'btn btn-secondary btn-sm float-end', 'escape' => false]) ?>
        </div>

        <div class="card-body">
            <div class="fooRatings index content">
                <div class="table-responsive">
                    <table class="table table-bordered table-sm dataset" style="width:100%">
                        ...
                        <tr class="filter-headers">
                            ...
                            <th data-db-type="<?= $typeMap['score'] ?>"><?= Inflector::humanize('score') ?></th>

 ...
 * @var AppView $this
 * @var array $typeMap
 * @var Subscription[]|CollectionInterface $subscriptions
...
$this->set('headerTitle', __('Subscriptions'));
...
<?php foreach ($subscriptions as $subscription): ?>
    <?php $rows[$counter][] = $this->Number->format($subscription->id) ?>
    <?php $rows[$counter][] = h($subscription->name) ?>
    <?php $rows[$counter][] = $this->Text->truncate(h($subscription->description) ?? '', 50) ?>
    <?php
    $appendName = ($subscription->name) ? ": $subscription->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$subscription->id}{$appendName}",
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Subscription Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Subscription'), ['action' => 'add'],

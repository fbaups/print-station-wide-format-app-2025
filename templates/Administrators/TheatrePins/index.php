 ...
 * @var AppView $this
 * @var array $typeMap
 * @var TheatrePin[]|CollectionInterface $theatrePins
...
$this->set('headerTitle', __('Theatre Pins'));
...
$counter = 0;
?>
<?php foreach ($theatrePins as $theatrePin): ?>
    <?php $rows[$counter][] = $this->Number->format($theatrePin->id) ?>
    <?php $rows[$counter][] = h($theatrePin->name) ?>
    <?php $rows[$counter][] = h($theatrePin->pin_code) ?>
    <?php $rows[$counter][] = $theatrePin->user_link === null ? '' : $this->Number->format($theatrePin->user_link) ?>
    <?php
    $appendName = ($theatrePin->name) ? ": $theatrePin->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$theatrePin->id}{$appendName}",
        'data-record-id' => $theatrePin->id,
    ];
    $rows[$counter][] =
        $this->Html->link(__('Preview'), "#", $previewLinkOptions) . " | " .
        $this->Html->link(__('View'), ['action' => 'view', $theatrePin->id]) . " | " .
        $this->Html->link(__('Edit'), ['action' => 'edit', $theatrePin->id]) . " | " .
        $this->Form->postLink(__('Delete'), ['action' => 'delete', $theatrePin->id], ['confirm' => __('Are you sure you want to delete # {0}?', $theatrePin->id)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Theatre Pin Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Theatre Pin'), ['action' => 'add'],
                ['class' => 'btn btn-secondary btn-sm float-end', 'escape' => false]) ?>
        </div>

        <div class="card-body">
            <div class="theatrePins index content">
                <div class="table-responsive">
                    <table class="table table-bordered table-sm dataset" style="width:100%">
                        ...
                        <tr class="filter-headers">
                            ...
                            <th data-db-type="<?= $typeMap['pin_code'] ?>"><?= Inflector::humanize('pin_code') ?></th>
                            <th data-db-type="<?= $typeMap['user_link'] ?>"><?= Inflector::humanize('user_link') ?></th>

 ...
 * @var AppView $this
 * @var array $typeMap
 * @var FooMethod[]|CollectionInterface $fooMethods
...
$this->set('headerTitle', __('Foo Methods'));
...
$counter = 0;
?>
<?php foreach ($fooMethods as $fooMethod): ?>
    <?php $rows[$counter][] = $this->Number->format($fooMethod->id) ?>
    <?php $rows[$counter][] = $fooMethod->has('foo_recipe') ? $this->Html->link($fooMethod->foo_recipe->name, ['controller' => 'FooRecipes', 'action' => 'view', $fooMethod->foo_recipe->id]) : '' ?>
    <?php $rows[$counter][] = $fooMethod->rank === null ? '' : $this->Number->format($fooMethod->rank) ?>
    <?php $rows[$counter][] = h($fooMethod->text) ?>
    <?php
    $appendName = ($fooMethod->name) ? ": $fooMethod->name" : "";
    $previewLinkOptions = [
        'class' => '',
        'data-bs-toggle' => "modal",
        'data-bs-target' => "#previewRecord",
        'data-record-title' => "Record #{$fooMethod->id}{$appendName}",
        'data-record-id' => $fooMethod->id,
    ];
    $rows[$counter][] =
        $this->Html->link(__('Preview'), "#", $previewLinkOptions) . " | " .
        $this->Html->link(__('View'), ['action' => 'view', $fooMethod->id]) . " | " .
        $this->Html->link(__('Edit'), ['action' => 'edit', $fooMethod->id]) . " | " .
        $this->Form->postLink(__('Delete'), ['action' => 'delete', $fooMethod->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fooMethod->id)]);
    ?>
    <?php $counter++ ?>
<?php endforeach; ?>
<?php
...
<div class="container-fluid px-4">
    <div class="card">
        ...
        <div class="card-header">
            <?= __('Foo Method Listing') ?>
            <?= $this->Html->link('<i class="fas fa-plus"></i>' . __('&nbsp;New Foo Method'), ['action' => 'add'],
                ['class' => 'btn btn-secondary btn-sm float-end', 'escape' => false]) ?>
        </div>

        <div class="card-body">
            <div class="fooMethods index content">

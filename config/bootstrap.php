...
if ($SettingsTable) {
    $trustProxyDbSetting = asBool($SettingsTable->getSetting('trust_proxy_header'));
} else {
    $trustProxyDbSetting = false;
}
...
// 1) force the base url from a hard coded value in the config, not preferred
$fullBaseUrl = Configure::read('App.fullBaseUrl');

// 2) try to extract from the environment or the trusted proxy/balancer
if (!$fullBaseUrl) {
    /*
     ...
     */
    $trustProxy = $trustProxyDbSetting;
...
// 3) last attempt, set a full base url from the DB
if (!$fullBaseUrl) {
    if ($SettingsTable) {
        $fullBaseUrl = $SettingsTable->getSetting('default_domain');
    }
}

//finally push it to the Router for application use
if ($fullBaseUrl) {

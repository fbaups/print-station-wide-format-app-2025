...
use App\OrderManagement\ArtifactGroupOrdering;
...
class ArtifactsController extends AppController
{
    ...
    public function view($id = null)
    {
        ...
        $videoMimeTypes = $this->Artifacts->getVideoMimeTypes();
        $audioMimeTypes = $this->Artifacts->getAudioMimeTypes();

        $this->Artifacts->populateEmptyArtifactMetadataExif($artifact);

        $this->set(compact('artifact', 'pdfMimeTypes', 'imageMimeTypes', 'videoMimeTypes', 'audioMimeTypes'));
    }
     ...
     */
    public function mobileUploadQrCode()
    {
        ...
        if ($this->request->isAll(['post'])) {
            ...
            $convertToOrderUrl = ['prefix' => 'Administrators', 'controller' => 'Artifacts', 'action' => 'mobile-upload-order', $seed->token];
            $convertToOrderUrl = Router::url($convertToOrderUrl, true);

            $this->set('pushUrl', $pushUrl);
            $this->set('convertToOrderUrl', $convertToOrderUrl);
            $this->set('seed', $seed);
    ...
    /**
     * Convert mobile uploads into an Order
     *
     * @param $artifactGrouping
     * @return Response|void
     */
    public function mobileUploadOrder($artifactGrouping = null)
    {
        if (empty($artifactGrouping)) {
            return $this->redirect(['action' => 'index']);
        }

        $artifacts = $this->Artifacts->find('all')->where(['grouping' => $artifactGrouping]);

        if ($artifacts->count() === 0) {
            $this->Flash->error(__('Cannot create an Order from the supplied Artifact grouping key. Please, try again.'));
            return $this->redirect(['action' => 'index']);
        }

        $ArtifactGroupOrdering = new ArtifactGroupOrdering();
        $tallies = $ArtifactGroupOrdering->loadOrder($artifactGrouping);

        if ($tallies['order_count'] > 0) {
            $this->Flash->success(__('Order created.'));
        } else {
            $this->Flash->error(__('Failed to create an Order.'));
        }

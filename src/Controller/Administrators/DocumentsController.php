...
class DocumentsController extends AppController
{
     ...
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function download($id = null, $format = 'json')
    {
        if (is_numeric($id)) {
            $id = intval($id);
            $errand = $this->Documents->downloadDocumentErrand($id);

            if ($errand) {
                $this->Flash->success(__('An Errand to download the Document has been created. Please check back in a few minutes.'));
            } else {
                $this->Flash->error(__('Could not create an Errand to download the Document. Please try again.'));

            }
        } else {
            $this->Flash->error(__('Please select a Document to download.'));
        }

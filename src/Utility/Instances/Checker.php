...
class Checker
{
    ...
    public function checkUncSettings($settings)
    {
        ...
        $readResult = file_get_contents_guzzle($repoUrlFile, $options);
        $deleteResultFile = false;
        $deleteResultFileCounter = 0;
        $deleteResultFolder = false;
        $deleteResultFolderCounter = 0;
        while (!$deleteResultFile && $deleteResultFileCounter <= 5) {
            $deleteResultFile = @unlink($repoUncFile);
            $deleteResultFileCounter++;
            usleep(20000); //sleep for 20ms
        }
        while (!$deleteResultFolder && $deleteResultFolderCounter <= 5) {
            $deleteResultFolder = @rmdir($repoUncPath);
            $deleteResultFolderCounter++;
            usleep(20000); //sleep for 20ms
        }
    ...
    public function checkSftpSettings($settings)
    {
        ...
        $options = ['verify' => false];
        $readResult = file_get_contents_guzzle($rndUrlFile, $options);
        if (!$readResult) {

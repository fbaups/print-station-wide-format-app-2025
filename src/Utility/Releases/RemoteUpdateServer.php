...
class RemoteUpdateServer
{
    ...
    public function checkUrlServer()
    {
        ...
        try {
            ...
            $urlPath = [
                'http_host' => TextFormatter::makeDirectoryTrailingForwardSlash($this->remote_update_url),
            ];
            return $UrlInspector->inspectUrlConnection($urlPath);
    ...
    public function checkB2Server()
    {
        ...
        //known issue in \vendor\zaxbux\backblaze-b2-php\src\Operations\FileOperationsTrait.php
        $fileToFix = ROOT . "\\vendor\\zaxbux\\backblaze-b2-php\\src\\Operations\\FileOperationsTrait.php";
        $fileContents = file_get_contents($fileToFix);
        $in = [
        ...
        $fileContents = str_replace($in, $out, $fileContents);
        file_put_contents($fileToFix, $fileContents);
        /* END_BLOCKED_CODE */
        ...
        //read test
        try {
            $listing = $this->filesystemB2->listContents($baseDir, false);

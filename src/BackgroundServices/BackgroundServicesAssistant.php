...
class BackgroundServicesAssistant
{
    ...
    /**
     * Create batch files that can be executed to install the Background Services into Windows
     *
     * @param array $options
     * @return bool
     */
    public function createBackgroundServicesBatchFiles(array $options = []): bool
    {
        $defaultOptions = [
            'php_version' => '',
            'username' => '',
        ...
        $errand_background_service_limit = $options['errand_background_service_limit'];
        $message_background_service_limit = $options['message_background_service_limit'];
        $purge_repo_limit = $options['database_purger_background_service_limit'];
        $hot_folder_background_service_limit = $options['hot_folder_background_service_limit'];
        $scheduled_task_background_service_limit = $options['scheduled_task_background_service_limit'];

        /** @var SettingsTable $Settings */
        $Settings = TableRegistry::getTableLocator()->get('Settings');
        $Settings->setSetting('errand_background_service_limit', $errand_background_service_limit);
        $Settings->setSetting('message_background_service_limit', $message_background_service_limit);
        $Settings->setSetting('database_purger_background_service_limit', $purge_repo_limit);
        $Settings->setSetting('hot_folder_background_service_limit', $hot_folder_background_service_limit);
        $Settings->setSetting('scheduled_task_background_service_limit', $scheduled_task_background_service_limit);
        ...
        //try and install the services now
        $runCommands = $commandsInstall;
        if ($this->isWindowsAdmin()) {
            $installedServices = $this->_getServices();
            $removeCommands = [];
            foreach ($installedServices as $installedService) {
                $removeCommands[] = __("net stop \"{0}\"", $installedService['name']);
                $removeCommands[] = __("\"{0}\" remove \"{1}\" confirm", $nssm, $installedService['name']);
            }

            $runCommands = array_merge($removeCommands, $runCommands);
        ...
        //generate a cmd file to boot services manually
        $commandsTest = implode("\r\n", $commandsTest) . "\r\npause\r\n";
        $commandsTest = str_replace(' """', ' "', $commandsTest);
        $commandsTest = str_replace('""" ', '" ', $commandsTest);
        file_put_contents($serviceCommandsTestFile, $commandsTest);


        //test to see if all appears to be installed as expected
        ...
        if (json_encode($newlyInstalledServices) == json_encode($expectedServicesNames)) {
            foreach ($newlyInstalledServices as $newlyInstalledService) {
                $this->addSuccessAlerts(__("Service \"{0}\" installed successfully!", $newlyInstalledService));
            }
            return true;
        } else {
        ...
        //generate CMD files to run installation manually
        $commandsInstall = implode("\r\n", $commandsInstall) . "\r\npause\r\n";
        $saveResultInstall = file_put_contents($serviceInstallFile, $commandsInstall);
     ...
     * @return false|void
     */
    public function uninstallBackgroundServices()
    {
        if ($this->isWindowsAdmin()) {
            $this->BackgroundServices->deleteAll(['id <> 0']);
            ...
            return count($installedServices);
        } else {
            return false;
        }
    }

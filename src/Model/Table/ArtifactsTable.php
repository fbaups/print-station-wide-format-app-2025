...
class ArtifactsTable extends AppTable
{
    ...
    public function createArtifact(array $data): false|Artifact
    {
        ...
        if ($artifactIsNew) {
            ...
            $chunks = $this->str_split_random($token);
            $pathSalt = sha1(Security::randomBytes(2048));
            $url = implode('/', $chunks) . "/{$pathSalt}/";
            $unc = implode('\\', $chunks) . "\\{$pathSalt}\\";
        ...
        if ($saveEntityResult) {
            ...
            $this->addDangerAlerts(json_encode($artifact->getErrors()));
            return false;
        }
        ...
        $artifactMetadata->exif = null;
        $artifactMetadata->width = 0;
        $artifactMetadata->height = 0;
        $artifactMetadata->duration = 0;
        ...
        $videoMimeTypes = $this->getVideoMimeTypes();
        if (in_array($artifact->mime_type, $videoMimeTypes)) {
            $FFmpeg = new FFmpegCommands();
            $analysis = $FFmpeg->videoAnalyse($dest);
            if ($analysis) {
                $artifactMetadata->exif = $analysis;
                $artifactMetadata->width = $FFmpeg->videoWidth($dest);
                $artifactMetadata->height = $FFmpeg->videoHeight($dest);
                $artifactMetadata->duration = $FFmpeg->videoDuration($dest);
            }
        }

        $artifactMetadata = $this->ArtifactMetadata->save($artifactMetadata);
        if ($artifactMetadata) {
    ...
    {
        $PrePressCommands = GetCommands::getPrepressCommands('callas');
        if (!$PrePressCommands) {
        ...
        $toolVersion = $PrePressCommands->getCliVersion();

        unset($report['aggregated']['file']['path'], $report['aggregated']['file']['filepath'], $report['aggregated']['doc']['xmpmetadata']);
        ...
        $meta['tool'] = $toolVersion;
        $meta['env'] = $report['aggregated']['env'] ?? [];
    ...
    /**
     * Convenience function to create an Artifact from an UploadedFile
     *
     * @param UploadedFile $uploadedFile
     * @param array $data
     * @return Artifact|false
     */
    public function createArtifactFromUploadedFile(UploadedFile $uploadedFile, array $data = []): false|Artifact
    {
        $token = sha1(Security::randomBytes(1600));
        ...
        $data['token'] = $token;
        $data['blob'] = $uploadedFile->getStream()->getContents();
    ...
    public function createSampleSizes($artifactOrIdOrToken, array $params = []): bool
    {
        $defaultParams = [
            ...
            'mr' => true,
            'hr' => true,
            'video_still_format' => 'jpg', //when converting videos only use JPG as PNG sample previews will not be found.
            'video_still_time' => 0.5, //when converting videos get 1/2 second in as usually first frame is black.
        ];
        ...
        $detector = new FinfoMimeTypeDetector();
        $mimeType = $artifact->mime_type ?? $detector->detectMimeTypeFromFile($artifact->full_unc);
        ...
        if (!$isPdf && !$isImage && !$isVideo) {
            $this->addDangerAlerts("Samples cannot be created because the Artifact is not a valid Image, PDF or Video.");
            return false;
        }
        ...
        if ($isImage) {
            ...
            if ($result) {
                ...
                $this->addDangerAlerts("Failed to create Image samples.");
            }
        }

        //process samples for Video
        if ($isVideo) {
            $result = $this->sampleImagesFromVideo($artifact, $params, $sizes, $baseSavePath);
            if ($result) {
                $this->addSuccessAlerts("Created Video samples.");
            } else {
                $this->addSuccessAlerts("Failed to create Video samples.");
            }
        }
    ...
    private function sampleImagesFromVideo(Artifact $artifact, $params, $sizes, $baseSavePath): bool
    {
        $FFmpegCommands = new FFmpegCommands();
        if (!$FFmpegCommands->isAlive()) {
            $this->addDangerAlerts("FFmpeg Commands are unavailable.");
            return false;
        }
        ...
        //make 1 still image from a frame in the video
        $stillTime = $params['video_still_time'] ?? 0;
        $stillFormat = $params['video_still_format'] ?? 'jpg';
        $videoFilenameNoExt = pathinfo($artifact->full_unc, PATHINFO_FILENAME);
        $stillPath = "{$baseSavePath}{$videoFilenameNoExt}.{$stillFormat}";
        $result = $FFmpegCommands->videoThumbnail($artifact->full_unc, $stillPath, $stillTime);
        if (!$result) {
            $this->addDangerAlerts("FFmpeg could not create a still image from the video.");
            return false;
        }
        ...
        foreach ($sizes as $sizeName => $sizeValue) {
            try {
                ...
                //check if sample already exists
                $fileNameNoExt = pathinfo($stillPath, PATHINFO_FILENAME);
                $ripFormat = pathinfo($stillPath, PATHINFO_EXTENSION);
                $fullPath = "{$baseSavePath}{$fileNameNoExt}_{$sizeName}.{$ripFormat}";
                if (is_file($fullPath)) {
                    $this->addSuccessAlerts("Skipping {$sizeName} sample as it already exists.");
                    continue;
     ...
     * @return array
     */
    public function str_split_random(string $string = '', int $minLength = 1, int $maxLength = 5): array
    {
        $l = strlen($string);
        $i = 0;
    ...
    public function deleteByGrouping(string $grouping): bool
    {
        ...
        $artifactCounter = 0;
        $artifactDeletedCounter = 0;

        foreach ($artifacts as $artifact) {
            if ($artifact) {
                $artifactCounter++;
                if ($this->delete($artifact)) {
                    $artifactDeletedCounter++;
                }
            }
        }

        if ($artifactCounter === $artifactDeletedCounter) {
            return true;
        }
    ...
    /**
     * Upgrade legacy UNC paths to prevent brute force attacks on static link.
     *
     * @param int $limit
     * @return int
     */
    public function findAndSecureArtifactUncPaths(int $limit = 200): int
    {
        $hardLimit = 500;
        $finalLimit = min($limit, $hardLimit);

        $artifactsSecure = $this->find('all')
            ->limit($finalLimit);
        ...
        $dbDriver = ($this->getConnection())->getDriver();
        if ($dbDriver instanceof Sqlite) {
            $artifactsSecure->where(function (QueryExpression $exp, $q) use ($tooShort) {
                return $exp->lt($q->func()->length(['unc' => 'identifier']), $tooShort);
            });
        } elseif ($dbDriver instanceof Mysql) {
            $artifactsSecure->where(function (QueryExpression $exp, $q) use ($tooShort) {
                return $exp->lt($q->func()->LENGTH(['unc' => 'identifier']), $tooShort);
            });
        } else {
            $artifactsSecure->where(function (QueryExpression $exp, $q) use ($tooShort) {
                return $exp->lt($q->func()->LEN(['unc' => 'identifier']), $tooShort);
            });
        }
         ...
         * @var Artifact $artifact
         */
        foreach ($artifactsSecure->toArray() as $artifact) {
            if ($this->secureArtifactUnc($artifact)) {
                $counter++;
            }
        }
    ...
    /**
     * @param int|Artifact $idOrEntity
     * @return bool
     */
    public function secureArtifactUnc(int|Artifact $idOrEntity): bool
    {
        /** @var Artifact $artifact */
        $artifact = $this->asEntity($idOrEntity);

        if (strlen($artifact->unc) > 80) {
            //already long enough
            return true;
        }

        $originalFileUnc = $artifact->full_unc;
        $originalDirname = TextFormatter::makeDirectoryTrailingSmartSlash(pathinfo($originalFileUnc, PATHINFO_DIRNAME));

        $pathSalt = sha1(Security::randomBytes(2048));

        if ($this->moveContentsToSubDirectory($originalDirname, $pathSalt)) {
            $artifact->unc = TextFormatter::makeDirectoryTrailingSmartSlash($artifact->unc . $pathSalt);
            $artifact->url = TextFormatter::makeDirectoryTrailingSmartSlash($artifact->url . $pathSalt);
            $this->save($artifact);
    ...
    /**
     * Take everything in the given directory and move it into the given subdirectory
     *
     * @param string $directory
     * @param string $subDirectoryName
     * @return bool
     */
    public function moveContentsToSubDirectory(string $directory, string $subDirectoryName): bool
    {
        try {

            $directory = TextFormatter::makeDirectoryTrailingSmartSlash($directory);
            $ds = substr($directory, -1);

            $adapter = new LocalFilesystemAdapter($directory);
            $filesystem = new Filesystem($adapter);
            $listing = $filesystem->listContents('', false)->sortByPath();
            $files = [];
            $folders = [];
            ...
            foreach ($folders as $folder) {
                $filesystem->move($folder, "{$subDirectoryName}{$ds}{$folder}");
            }

            foreach ($files as $file) {
                $filesystem->move($file, "{$subDirectoryName}{$ds}{$file}");
            }
    ...
    public function getDefaultPdfConversionImageProperties(): array
    {
        return [
            'format' => 'jpg',
            'quality' => 100,
    ...
    public function getDefaultPdfConversionPageProperties(): array
    {
        return [
            'unit' => 'mm',
            'page_width' => '210',
    ...
    /**
     * Create a PDF version of the Artifact
     *
     * @param Artifact|int $artifact
     * @param array $imageProperties
     * @param array $pageProperties
     * @param bool $forceRefresh
     * @return string|false
     */
    public function createVersionPdf(Artifact|int $artifact, array $imageProperties, array $pageProperties, bool $forceRefresh = false): string|false
    {
        $imageProperties = array_merge($this->getDefaultPdfConversionImageProperties(), $imageProperties);
        $pageProperties = array_merge($this->getDefaultPdfConversionPageProperties(), $pageProperties);

        /** @var Artifact $artifact */
        $artifact = $this->asEntity($artifact);

        $pdfSavePath = "{$artifact->full_unc}.pdf";

        if (!$forceRefresh && is_file($pdfSavePath)) {
            return $pdfSavePath;
        }

        $IW = new ImageWrapper();
        $pdf = $IW->wrapImage($artifact->full_unc, $imageProperties, $pageProperties);

...
//            ->notEmpty('old_password');
     ...
     * @return int
     */
    public function getUserRolesSessionTimeoutSeconds($id = null): int
    {
        $timeout = 0;
        $userData = $this->getUserSessionDataForCache($id);
        $user = $this->asEntityWithRoles($userData['id']);
    ...
    public function sendApprovedEmail(User $user)
    {
        ...
        //schedule an email
        $options = [
            'url' => ['controller' => 'users', 'action' => 'confirm', '{token}'],
            'expiration' => new DateTime('+ 1 hour'),
            'user_link' => $user->id,
        ];
        $token = $this->Seeds->createSeedReturnToken($options);
        ...
        $data = [
            ...
            'view_vars' => [
                ...
                'token' => $token,
                'url' => Router::url(['controller' => 'users', 'action' => 'confirm', $token], true),
            ],
            'email_to' => [$user->email => $user->full_name],
            'email_from' => $this->Settings->getEmailForCakePhpMailer(),
    ...
    public function sendDeniedEmail(User $user)
    {
        ...
        $data = [
            ...
            'view_vars' => [
                ...
                'url' => Router::url(['controller' => 'pages', 'action' => 'contact'], true),
    ...
    public function sendAdminApprovalRegistrationLink(mixed $data): bool|User
    {
        ...
        //schedule an email
        $options = [
            'url' => ['controller' => 'users', 'action' => 'approve', '{token}'],
            'expiration' => new DateTime('+ 1 year'),
            'user_link' => $newUser->id,
        ];
        ...
        $options = [
            'url' => ['controller' => 'users', 'action' => 'deny', '{token}'],
            'expiration' => new DateTime('+ 1 year'),
            'user_link' => $newUser->id,
        ];
        ...
        foreach ($allAdmins as $admin) {
            ...
            $messageData = [
                ...
                'view_vars' => [
                    ...
                    'url_approve' => Router::url(['controller' => 'users', 'action' => 'approve', $tokenApprove], true),
                    'url_deny' => Router::url(['controller' => 'users', 'action' => 'deny', $tokenDeny], true),
                    'url_edit' => Router::url(['controller' => 'users', 'action' => 'edit', $newUser->id], true),
                ],

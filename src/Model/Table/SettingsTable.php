...
class SettingsTable extends AppTable
{
    ...
    public function checkRepositoryDetails(bool $forceRefresh = false): array
    {
        ...
        $sftpRoundTripSettings = [
            'http_host' => $repo_url,
            'sftp_host' => $repo_sftp_host,
            'sftp_port' => $repo_sftp_port,
            'sftp_username' => $repo_sftp_username,
            'sftp_password' => $repo_sftp_password,
            'sftp_timeout' => $repo_sftp_timeout,
            'sftp_path' => $repo_sftp_path,
        ];
        ...
        $UncInspector = new UncInspector();
        $SftpInspector = new SftpInspector();
        $UrlInspector = new UrlInspector();

        $isURL = $UrlInspector->inspectUrlConnection($urlSettings);
        $isUNC = $UncInspector->inspectUncServer($uncRoundTripSettings);
        if (!$isUNC) {
            $isSFTP = $SftpInspector->inspectSftpServer($sftpRoundTripSettings);
        } else {
            $isSFTP = null;
        }

        $this->mergeAlertsFromObject($UrlInspector);
        $this->mergeAlertsFromObject($UncInspector);
        ...
        $cachedDetails = [
            ...
            'isSFTP' => $isSFTP,
            'isUNC' => $isUNC,
            'remoteUpdateDebug' => $this->getAllAlertsLogSequence(),
        ];

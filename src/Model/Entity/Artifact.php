...
class Artifact extends Entity
{
    ...
    protected function _getFullUrl()
    {
        ...
        if ($this->repoMode === 'static') {
            ...
            $str = Router::url("/", true) . "artifacts/fetch/{$this->token}/{$this->name}";
        }
    ...
    protected function _getLightTableUrls(): array
    {
        ...
        if (is_array($this->lightTableUrlsCache)) {
            return $this->lightTableUrlsCache;
        }
        ...
        foreach ($pageRange as $pageNumber) {
            ...
            if (is_file($uncPath)) {
                ...
                if ($this->repoMode === 'static') {
                    ...
                    $urls[$pageNumber] = Router::url("/", true) . "artifacts/fetch/{$this->token}/{$name}";
                }
    ...
    private function generateSampleUrl(string $sizeName = 'thumbnail'): string
    {
        ...
        if (strtolower($fileExt) === 'pdf') {
            $fileExt = 'png'; //PDF files will always be converted to PNG
        }
        ...
        if ($this->repoMode === 'static') {
            ...
            $str = Router::url("/", true) . "artifacts/sample/{$this->token}/{$sizeName}/{$this->name}";
        }

...
class ServiceScheduledTasksCommand extends ServiceCommand
{
    ...
    private function doLoop()
    {
        ...
        while ($currentTime->lessThanOrEquals($retirementDate)) {
            ...
            if (!isset($codeSweeperLastRunTime)) {
                $runSweeperFlag = true;
            } else {
                $codeSweeperCurrentRunTime = new DateTime();
                $codeSweeperNextRunTime = (clone $codeSweeperLastRunTime)->addMinutes($sweeperDelayMins);

                if ($codeSweeperCurrentRunTime->greaterThanOrEquals($codeSweeperNextRunTime)) {
                    $runSweeperFlag = true;
                } else {
                    $runSweeperFlag = false;
                }
            }
            ...
            if ($runSweeperFlag) {
                ...
                foreach ($projects as $project) {
                    ...
                    $this->io->out(__("Logged {0} files that have changed in Project {1}.", $rowCount, $project->name));
                }

                $codeSweeperLastRunTime = new DateTime();
            } else {
                $this->io->out(__("Waiting for sweeper delay of {0} minutes.", $sweeperDelayMins));
            }
            //===============================================================================
            /* END_BLOCKED_CODE */

...
class ServiceDatabasePurgerCommand extends ServiceCommand
{
    ...
    private function doLoop()
    {
        ...
        //START do these once per boot
        $this->IntegrationCredentials->updateLastStatusForAll();
        //END do these once per boot
        ...
        while ($currentTime->lessThanOrEquals($retirementDate)) {
            ...
            //===Artifacts Metadata Orphans==================================================
            $timerStart = new DateTime();
            $orphanedDeletionCount = $this->Artifacts->ArtifactMetadata->deleteOrphaned();
            $timerEnd = new DateTime();
            $timerDiff = $timerEnd->diffInSeconds($timerStart);
            $msg = __("Deleted {0} Artifact Metadata in {1} seconds.", $orphanedDeletionCount, $timerDiff);
            $this->io->out($msg);
            ...
            //===Secure Artifacts===========================================================
            $timerStart = new DateTime();
            $secureArtifactsCount = 0;

            foreach (range(1, 1000) as $loop) {
                $secureArtifactsCount += $this->Artifacts->findAndSecureArtifactUncPaths();
            }
            $activityCounter = $activityCounter + $secureArtifactsCount;

            $timerEnd = new DateTime();
            $timerDiff = $timerEnd->diffInSeconds($timerStart);
            $msg = __("Increased security on {0} Artifacts in {1} seconds.", $secureArtifactsCount, $timerDiff);
            $this->io->out($msg);

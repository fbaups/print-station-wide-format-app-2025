    ...
    private function doLoop()
    {
        ...
        while ($currentTime->lessThanOrEquals($retirementDate)) {
            ...
            foreach ($allHotFoldersKeyed as $hotFolder) {
                //check if folder has read/write/delete permissions
                $settings = ['unc_host' => $hotFolder->path];
                if (!$UncInspector->inspectUncServer($settings)) {
                    continue;
                }
                ...
                $hotFolderPath = TextFormatter::makeDirectoryTrailingSmartSlash($hotFolder->path);
                ...
                foreach ($entries as $entry) {
                    ...
                    if (is_dir($fullPath)) {
                        $fullPath = TextFormatter::makeDirectoryTrailingSmartSlash($fullPath);
                    }

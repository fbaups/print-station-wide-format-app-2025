 ...
 */
class ProcessPdfOrders extends WorkflowBase
{
    /**
    ...
    public function execute(int|HotFolderEntry $hotFolderEntry, array $options = []): bool
    {
        ...
        if (is_file($hotFolderEntry->path)) {
            /*
             * Single pdf in the top folder is not allowed, move to a sub-folder and update $hotFolderEntry
        ...
        if (is_dir($hotFolderEntry->path)) {
            ...
            $orderOptions = [
                ...
                'description' => "PDF Folder - {$description}",
                'external_order_number' => $topDirectoryName,
            ];
            $order = $PdfOrdering->loadOrder($tmpFolderOrder, $orderOptions);
            ...
            $this->mergeAlerts($PdfOrdering->getAllAlertsForMerge());
            return true;
        }
